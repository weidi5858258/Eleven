    ...
    protected Bitmap loadTopSongs(Cursor sortedCursor) {
        ...
        if (loadedBitmaps.size() > 0) {
            ...
            if (loadedBitmaps.size() == MAX_NUM_BITMAPS_TO_LOAD) {
                ...
                Class combinedCanvasClass = combinedCanvas.getClass();
                if(combinedCanvasClass != null){
                    try {
                        Method method = combinedCanvasClass.getDeclaredMethod("release");
                        method.setAccessible(true);
                        method.invoke(combinedCanvas);
