...
public class LocaleSetManager {
    ...
    public boolean localeSetNeedsUpdate() {
        ...
        String icuVersion = "";
        try {
            Class icuClass = Class.forName("libcore.icu.ICU");
            if(icuClass != null){
                Method method = icuClass.getDeclaredMethod("getIcuVersion");
                method.setAccessible(true);
                icuVersion = (String) method.invoke(icuClass);
            }
        ...
        if (!icuVersion.equals(storedICUversion)) {
            Log.d(TAG, "ICU version has changed from: " + storedICUversion + " to " + icuVersion);
            return true;
        }
